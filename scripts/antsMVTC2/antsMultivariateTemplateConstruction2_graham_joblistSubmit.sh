      ...
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            then
            id=`sbatch --job-name=antsrigid --export=ANTSPATH=$ANTSPATH $QSUBOPTS --nodes=1 --cpus-per-task=4 --time=${WALLTIME} --mem=${MEMORY} $qscript | rev | cut -f1 -d\ | rev`
#		echo "$qscript" >> ${outdir}/joblist.0.1_rigid # this is 
 #           jobIDs="$jobIDs $id"
  #          sleep 2.0
        elif [[ $DOQSUB -eq 0 ]];
          then
    ...
    if [[ BACKUPEACHITERATION -eq 1 ]];
      then
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
echo            mv ${outdir}/slurm-*.out ${outdir}/rigid/
    echo        mv ${outdir}/job*.txt ${outdir}/rigid/

            # Remove submission scripts
        echo    rm -f ${outdir}/job_${count}_qsub.sh
        fi
      else
      ...
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            echo -e "$SCRIPTPREPEND" >> $qscript
            echo -e "$exe" >> $qscript
         #   id=`sbatch --job-name=antsdef${i} --export=ANTSPATH=$ANTSPATH --nodes=1 --cpus-per-task=4 --time=${WALLTIME} --mem=${MEMORY} $QSUBOPTS $qscript | rev | cut -f1 -d\ | rev`
		text_i=`printf "%02d" $i`
		echo "$qscript" >> ${outdir}/joblist.${text_i}.01_deformable
          #  jobIDs="$jobIDs $id"
         #   sleep 2.0
        elif [[ $DOQSUB -eq 0 ]];
          then
...
# update after individual runs
    ...
    if [[ $BACKUPEACHITERATION -eq 1 ]];
      then
        ...
        echo "--------------------------------------------------------------------------------------"
        echo "mkdir ${outdir}/ANTs_iteration_${i} " >> $qscript
        echo "cp ${TEMPLATENAME}${j}warplog.txt ${outdir}/*.cfg ${OUTPUTNAME}*.nii.gz ${OUTPUTNAME}*.mat ${outdir}/ANTs_iteration_${i}/" >> $qscript
        # backup logs
        if [[ $DOQSUB -eq 1 ]];
            then
            ...
            then
            echo mv ${outdir}/slurm-*.out ${outdir}/ANTs_iteration_${i} >> $qscript
                text_i=`printf "%02d" $i`
                echo "$qscript" >> ${outdir}/joblist.${text_i}.02_shapeupdate
