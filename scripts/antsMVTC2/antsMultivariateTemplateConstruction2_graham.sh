      ...
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            then
            id=`sbatch --job-name=antsrigid --export=ANTSPATH=$ANTSPATH $QSUBOPTS --nodes=1 --cpus-per-task=32 --time=${WALLTIME} --mem=${MEMORY} $qscript | rev | cut -f1 -d\ | rev`
            jobIDs="$jobIDs $id"
            sleep 2.0
        elif [[ $DOQSUB -eq 0 ]];
          then
  ...
  do
      ...
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            echo -e "$SCRIPTPREPEND" >> $qscript
            echo -e "$exe" >> $qscript
            id=`sbatch --job-name=antsdef${i} --export=ANTSPATH=$ANTSPATH --nodes=1 --cpus-per-task=32 --time=${WALLTIME} --mem=${MEMORY} $QSUBOPTS $qscript | rev | cut -f1 -d\ | rev`
            jobIDs="$jobIDs $id"
            sleep 2.0
        elif [[ $DOQSUB -eq 0 ]];
          then
    ...
    if [[ $DOQSUB -eq 5 ]];
      then
        ...
        echo "--------------------------------------------------------------------------------------"
        # now wait for the stuff to finish - this will take a while so poll queue every 10 mins
        ${ANTSPATH}/waitForSlurmJobs.pl 1 1800 $jobIDs
        if [[ ! $? -eq 0 ]];
          then
            echo "SLURM submission failed - jobs went into error state"
            exit 1;
